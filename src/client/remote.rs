...
use std::sync::Mutex;
use std::thread;
use swagger::{AuthData, ContextBuilder, EmptyContext, Has, Push, XSpanIdString, make_context, make_context_ty};
...
impl Handle {
    pub fn new(url: &'static str) -> Handle {
        ...
        thread::spawn(move || {
            ...
            let i2cbus = Client::try_new_http(url)
...
fn handle_request_api(
    request: Request,
    i2cbus: &Client<hyper::client::ResponseFuture>,
